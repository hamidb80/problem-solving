...
proc debugMatrix[T](m: Matrix[T], slice: Slice[int]) =
  {.cast(nosideEffect).}:
    let ttab = newUnicodeTable()
    ttab.setHeaders @["y->x"] & mapIt(slice, $it)

    for y in slice:
      ttab.addRow @[$y] & slice.mapIt($m[y][it])

    printTable ttab

proc debugShortestFloyd(v: int, sm: ShortestPathGraph, cs: ShortestPathCursor) =
  let slice = 1..<sm.width.int
  debugEcho "=== Using Vertex ", v, " ==="
  debugEcho "shortest"
  debugMatrix sm, slice
  debugEcho "cursor"
  debugMatrix cs, slice
